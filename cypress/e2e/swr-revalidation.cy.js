...
it('should refresh the shows page after 30 seconds', () => {
  ...
  // advance time (less than 30 sec revalidate interval) and check again
  cy.tick(ONE_SECOND);
  cy.findAllByText(/sold out/i).should('have.length', 1);

  // advance clock 30 sec more; now additional sold out show should display
...
it('should refresh the reservations page after fifteen seconds', () => {
  ...
  // the first show from the fake data has an ID of 0
  // and 10 avalaible seats
  const newReservation = generateNewReservation({
    reservationId: 12345,
    showId: 0,
    seatCount: 2,
